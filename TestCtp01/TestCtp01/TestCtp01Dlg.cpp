	...
	: CDialogEx(CTestCtp01Dlg::IDD, pParent)
{
	...
	//IService* ctp_service = new CTPService();
}
...
BEGIN_MESSAGE_MAP(CTestCtp01Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(BUTTON_LOGIN, &CTestCtp01Dlg::OnBnClickedLogin)
	ON_BN_CLICKED(BUTTON_EXIT, &CTestCtp01Dlg::OnBnClickedExit)
END_MESSAGE_MAP()
...
void CTestCtp01Dlg::OnBnClickedLogin()
{
	...
	HWND user_name_hwnd = ::GetDlgItem(this->GetSafeHwnd(),EDIT_USERNAME);

	::GetWindowText(user_name_hwnd,user_name,128);
	MessageBox(user_name,_T("用户名"),MB_OK);

	
	HWND user_pwd_hwnd = ::GetDlgItem(this->GetSafeHwnd(),EDIT_PASSWORD);

	::GetWindowText(user_pwd_hwnd,use_pwd,128);
	MessageBox(use_pwd,_T("密码"),MB_OK);
}

void CTestCtp01Dlg::OnBnClickedExit()
{
	MessageBox(_T("点击退出")_T("信息"),MB_OK);
}
